...
class LayerNorm2d(nn.LayerNorm):
    ...
    def forward(self, x):
        ...
        bhwc = super(LayerNorm2d, self).forward(bhwc)
...
class Restormer(nn.Module):
    def __init__(
        ...
        self.encoder_level3 = RestormerBlock(
            int(dim * 2**2),
            heads[2],
            num_blocks[2],
            ffn_expansion_factor,
        ...
        self.decoder_level3 = RestormerBlock(
            int(dim * 2**2),
            heads[2],
            num_blocks[2],
            ffn_expansion_factor,
        ...
        self.decoder_level2 = RestormerBlock(
            int(dim * 2**1),
            heads[1],
            num_blocks[1],
            ffn_expansion_factor,
        ...
        self.decoder_level1 = RestormerBlock(
            int(dim * 2**1),
            heads[0],
            num_blocks[0],
            ffn_expansion_factor,
        ...
        self.refinement = RestormerBlock(
            int(dim * 2**1),
            heads[0],
        ...
        if not hook:
            ...
            if self.scale == 1:
                ...
                for output in self.output:
                    out_dec_level1_list.append(output(out_dec_level1) + inp_img)
                out_dec_level1 = torch.cat(out_dec_level1_list, dim=1)
                out_dec_level1 = F.pixel_shuffle(out_dec_level1, self.scale)

            return out_dec_level1


@ARCH_REGISTRY.register()
class Restormer_origin(nn.Module):
    def __init__(
        self,
        inp_channels=3,
        out_channels=3,
        ...
        super(Restormer_origin, self).__init__()
