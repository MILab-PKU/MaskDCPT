...
class NAFBlocks(nn.Module):
    def __init__(self, chan, num):
        super().__init__()
        self.blocks = nn.ModuleList([NAFBlock(chan) for _ in range(num)])
...
class NAFNetBaseline(nn.Module):
    def __init__(
        ...
        for num in enc_blk_nums:
            self.encoders.append(NAFBlocks(chan, num))
            self.downs.append(nn.Conv2d(chan, 2 * chan, 2, 2))
            chan = chan * 2

        self.middle_blks = NAFBlocks(chan, middle_blk_num)
...
class NAFNet(Local_Base, NAFNetBaseline):
    def __init__(self, *args, train_size=(1, 3, 384, 384), fast_imp=True, **kwargs):
