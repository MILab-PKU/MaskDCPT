    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt["type"] == "lmdb":
            ...
            self.paths = paired_paths_from_folder(
                ...
                ["lq", "gt"],
                self.filename_tmpl,
                datasets_balance=self.datasets_balance,
                multi=multi,
            )
...
class PairedImageDenoiseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # else: # Poisson
        #     noise = random_generate_poisson_noise(img_lq, self.scale_range, 0.4) # Following RealESRGAN
        #     img_lq = img_lq + noise
...
@DATASET_REGISTRY.register()
class PairedImageOutdoorRainDataset(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(PairedImageOutdoorRainDataset, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt["io_backend"]
        ...
        self.suffix = self.opt.get("suffix", ".png")
    ...
    def __getitem__(self, index):
        ...
        gt_name = lq_path.split("/")[-1].split("_")[0] + "_" + lq_path.split("/")[-1].split("_")[1] + self.suffix
        gt_path = os.path.join(self.gt_folder, gt_name)
        if self.decode:
            img_bytes = self.file_client.get(gt_path, "gt")
            img_gt = imfrombytes(
