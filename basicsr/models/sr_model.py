...
from basicsr.losses.basic_loss import rfft
...
from basicsr.utils.lr_decay import lr_wd_annealing, get_param_groups
...
class SRModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.layer_decay = opt.get("layer_decay", None)
        self.eval_lpips = opt.get("eval_lpips", False)
        ...
        if self.eval_lpips:
            self.iqa_metric = pyiqa.create_metric('lpips', device=self.device)
    ...
    def init_training_settings(self):
        ...
        if train_opt.get("hist_opt"):
            self.cri_hist = build_loss(train_opt["hist_opt"]).to(self.device)
        else:
            self.cri_hist = None
        ...
        if train_opt.get("patch_opt_3d_xd"):
            self.cri_patch_3d_xd = build_loss(train_opt["patch_opt_3d_xd"]).to(
                self.device
            )
        else:
    ...
    def setup_optimizers(self):
        ...
        if self.layer_decay is not None:
            optim_params, para_groups_dbg = get_param_groups(self.net_g.module, lr_scale=self.layer_decay)
            logger.info(f'[get_ft_param_groups] param groups = \n{json.dumps(para_groups_dbg, indent=2)}\n')
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.layer_decay is not None:
            lr_wd_annealing(self.optimizer_g)

        l_total = 0
        loss_dict = OrderedDict()

        # l_scale = F.l1_loss(pred_gt_mean, rgb2ycbcr_pt(self.gt, True).mean(dim=[1, 2, 3]))
        # l_total += l_scale
        # loss_dict["l_scale"] = l_scale

        # pixel loss
        if self.cri_pix:
            # l_pix = self.cri_pix(self.output, self.gt) + self.cri_pix(rgb2ycbcr_pt(self.output, True).mean(dim=[2, 3]), rgb2ycbcr_pt(self.gt, True).mean(dim=[2, 3]))
            l_pix = self.cri_pix(self.output, self.gt)
            l_total += l_pix
            loss_dict["l_pix"] = l_pix
        # hist loss
        if self.cri_hist:
            l_hist = self.cri_hist(self.output, self.gt)
            l_total += l_hist
            loss_dict["l_hist"] = l_hist
        # frequency loss
        if self.cri_fft:
            l_fft = self.cri_fft(rfft(self.output), rfft(self.gt))
            l_total += l_fft
        ...
        if self.cri_ldl:
            ...
            l_total += l_g_ldl.mean()
            loss_dict["l_ldl"] = l_g_ldl.mean()
        ...
        # loss_dict["psnr"] = calculate_psnr_pt(
        #     self.output,
        #     self.gt,
        #     crop_border=self.opt.get("scale", 1),
        #     test_y_channel=True,
        # ).mean()
        # loss_dict["ssim"] = calculate_ssim_pt(
        #     self.output,
        #     self.gt,
        #     crop_border=self.opt.get("scale", 1),
        #     test_y_channel=True,
    ...
    def test(self):
        if hasattr(self, "net_g_ema"):
            ...
            with torch.no_grad():
                ...
                # print(rgb2ycbcr_pt(self.gt, True).mean(dim=[1, 2, 3]) / rgb2ycbcr_pt(self.output.detach(), True).mean(dim=[1, 2, 3]))
                # self.output *= (pred_gt_mean / rgb2ycbcr_pt(self.output.detach(), True).mean(dim=[1, 2, 3]))
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.lq)
    ...
    def nondist_validation(
        ...
        if with_metrics:
            ...
            if self.eval_lpips:
                self.best_metric_results[dataset_name]["lpips"] = dict(better="lower", val=float("inf"), iter=-1)
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if self.eval_lpips:
                self.metric_results["lpips"] += self.iqa_metric(visuals["result"], visuals["gt"]).item()
